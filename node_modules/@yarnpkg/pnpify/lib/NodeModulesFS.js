...
class NodeModulesFS extends fslib_1.ProxiedFS {
    constructor(pnp, { realFs = fs_1.default, pnpifyFs = true } = {}) {
        super(fslib_2.npath);
        this.baseFs = new PortableNodeModulesFS(pnp, { baseFs: new fslib_1.NodeFS(realFs), pnpifyFs });
    }
    mapFromBase(path) {
...
class PortableNodeModulesFS extends fslib_1.FakeFS {
    constructor(pnp, { baseFs = new fslib_1.NodeFS(), pnpifyFs = true } = {}) {
        super(fslib_2.ppath);
        ...
        this.baseFs = baseFs;
        const { tree, errors } = buildNodeModulesTree_1.buildNodeModulesTree(pnp, this.options);
        if (!tree)
        ...
        this.pnpFilePath = fslib_2.ppath.join(pnpRootPath, fslib_3.Filename.pnpCjs);
        this.watchPnpFile(pnpRootPath);
    }
    watchPnpFile(pnpRootPath) {
        this.baseFs.watch(pnpRootPath, { persistent: false }, (_, filename) => {
            if (filename === fslib_3.Filename.pnpCjs) {
                const pnp = dynamicRequire_1.dynamicRequireNoCache(this.pnpFilePath);
                const nodeModulesTree = buildNodeModulesTree_1.buildNodeModulesTree(pnp, this.options);
                if (!nodeModulesTree)
